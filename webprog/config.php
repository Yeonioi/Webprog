...
define('DB_PASS', 'your_password'); // your SQL Server password
...
function processReferralSignup($conn, $newUserId, $referralCode) {
    ...
    if ($referrer) {
        ...
        ");
        $stmt->execute([$referrer['user_id'], $newUserId, $referralCode, POINTS_PER_SIGNUP]);
        ...
        $stmt = $conn->prepare("
            ...
            WHERE user_id = ?
        ");
        $stmt->execute([POINTS_PER_SIGNUP, $referrer['user_id']]);
        ...
        ");
        $stmt->execute([$referrer['user_id'], $referralId, POINTS_PER_SIGNUP]);
        
        // Update new user's referred_by
        $stmt = $conn->prepare("UPDATE users SET referred_by = ? WHERE user_id = ?");
        $stmt->execute([$referrer['user_id'], $newUserId]);
        
        // Check for milestone rewards
        checkMilestoneRewards($conn, $referrer['user_id']);
    }
}
...
function checkMilestoneRewards($conn, $userId) {
    ...
    $stmt = $conn->prepare("SELECT total_referrals FROM users WHERE user_id = ?");
    $stmt->execute([$userId]);
    ...
    foreach ($milestones as $milestone) {
        ...
        $stmt = $conn->prepare("
            SELECT reward_id FROM referral_rewards 
        ...
        if (!$stmt->fetch()) {
            // Award milestone
            $stmt = $conn->prepare("
                UPDATE users SET referral_points = referral_points + ? WHERE user_id = ?
            ");
            $stmt->execute([$milestone['points_reward'], $userId]);
            
            // Create reward record (use latest referral)
            $stmt = $conn->prepare("
                SELECT TOP 1 referral_id FROM referrals WHERE referrer_id = ? ORDER BY created_at DESC
            ");
            $stmt->execute([$userId]);
...
function getReferralStats($conn, $userId) {
    $stmt = $conn->prepare("
        SELECT 
            ...
            u.referral_points,
            u.total_referrals,
            COUNT(DISTINCT r.referral_id) as completed_referrals,
            ISNULL(SUM(rr.points), 0) as total_points_earned
        FROM users u
        LEFT JOIN referrals r ON u.user_id = r.referrer_id
        LEFT JOIN referral_rewards rr ON u.user_id = rr.user_id
        WHERE u.user_id = ?
        GROUP BY u.user_id, u.referral_code, u.referral_points, u.total_referrals
    ");
    $stmt->execute([$userId]);
    return $stmt->fetch(PDO::FETCH_ASSOC);
}
